...
@RestController
@Slf4j
@RequestMapping("/v1/portStatus")
public class PortStatusController {

	@Autowired
	private PortStatusServiceIface PortStatusService;

	@GetMapping("")
	@PreAuthorize("hasAnyRole('CC_RESP','TK_P','AVAYA_ADMIN','AVAYA_HOTLINE','TK_SV')")
	public ResponseEntity<Object> fetchPortStatus() {

		List<PortStatusDto> response = new ArrayList<>();

		log.info("request received to fetch port status");
		response = PortStatusService.fetchAllPortStatus();
