...
@Repository
public interface PbxSpecialPortRepository extends JpaRepository<PbxSpecialPort, Long> {

    @Query("SELECT CASE WHEN COUNT(p) > 0 THEN true ELSE false END " +
            "FROM pbx_special_port p " +
	...
	List<PbxSpecialPort> findByPbxSystem_id(Long idPbxSystem);
